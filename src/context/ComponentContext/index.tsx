import React, { useState, createContext, SyntheticEvent, MouseEvent } from 'react';
import {
...
const ComponentProvider = ({ children, ...props }: ComponentContextProps) => {
  ...
  const setOpenSnack = (isOpen: boolean) => setState(prevState => ({ ...prevState, isSnackOpen: isOpen }));

  const setSnackMessage = (message: string) => setState(prevState => ({ ...prevState, snackMessage: message }));
  ...
  const setDeviceModalOpen = (isModalOpen: boolean) => {
    setState(prevState => ({ ...prevState, isSelectDeviceModalOpen: isModalOpen }));
  };

  const toggleActivityDrawer = (isActivityDrawerOpen: boolean, activityLogsViewed: boolean) => {
    setState(prevState => ({
      ...prevState,
  ...
  const handleSelectDeviceModal = () => {
    setState(prevState => ({ ...prevState, isSelectDeviceModalOpen: !prevState.isSelectDeviceModalOpen }));
  };
  ...
  const handleCloseSnack = (event: SyntheticEvent | MouseEvent, reason?: string) => {
    if (reason === 'clickaway') {
      return;
    }
