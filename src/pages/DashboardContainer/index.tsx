...
const DashboardContainer: FunctionComponent<DashboardContainerProps> = (
	...
	const handleSelectDevice = () => {
		...
		props
			.activateDevice({ id: deviceId[0]._id })
			.then(async () => await props.getUserDetails());
		handleSelectDeviceModal();
	};
	...
	const handleChangeRole = (event: ChangeEvent<HTMLInputElement>) => {
		...
		props
			.editUserDetails(props.user._id, { role: roleId })
	...
	const photoImage = () => (
		...
		>
			<span className="mini-account-menu__image">
				{width > breakpoint && (
					<img className="mini-account-menu__image" src={photo} alt="image" />
				)}
			</span>
		</div>
	);
